...
QSize KColorButton::sizeHint() const
{
    ...
    return style()
        ->sizeFromContents(QStyle::CT_PushButton, &opt, QSize(40, 15), this)
...
QSize KColorButton::minimumSizeHint() const
{
    ...
    d->initStyleOption(&opt);
    return style()
        ->sizeFromContents(QStyle::CT_PushButton, &opt, QSize(3, 3), this)
        .expandedTo(QApplication::globalStrut());
}

void KColorButton::dragEnterEvent(QDragEnterEvent* event)
{
    event->setAccepted(qvariant_cast<QColor>(event->mimeData()->colorData()).isValid()
                       && isEnabled());
}
...
void KColorButton::keyPressEvent(QKeyEvent* e)
{
    if (e->matches(QKeySequence::Copy)) {
        ...
        const QColor& color = qvariant_cast<QColor>(
            QApplication::clipboard()->mimeData(QClipboard::Clipboard)->colorData());
        setColor(color);
    } else
